...
# Set ES_Malaga to NaN after 12:33:21
pmu_df.loc[pmu_df.index >= pd.to_datetime('2025-04-28 12:33:21').tz_localize('Europe/Madrid'), 'ES_Malaga'] = np.nan
...
fig = plots.create_frequency_plot(pmu_df, overview_t_start, overview_t_end, pmu_aliases, "Grid Frequency Measurements Across Europe", ymin=49.7, ymax=50.2)
fig.show()
fig.write_image(figures_dir / f"frequency_overview_{todays_suffix}.png", scale=1)
```
...
fig.show()
fig.write_image(figures_dir / f"frequency_oscillation1_{todays_suffix}.png")
```
...
fig.show()
fig.write_image(figures_dir / f"frequency_oscillation2_{todays_suffix}.png")
```
...
fig.show()
fig.write_image(figures_dir / f"frequency_dfd_{todays_suffix}.png")
```
...
fig = plots.create_frequency_plot(
    ...
    title_text = "Loss of Generation & Synchronism",
    ymin=49.63,
    ymax=50.05,
    events = events,
    lemur_x = 0.015,
...
fig.show()
fig.write_image(figures_dir / f"frequency_loss_and_separation_{todays_suffix}.png")
```
...
fig.show()
fig.write_image(figures_dir / f"rocof_closeup_{todays_suffix}.png")
```
...
fig = spec_data['fig']
fig.show()
fig.savefig(figures_dir / f"spectrogram_{pmu_name}_{todays_suffix}.png")
```
